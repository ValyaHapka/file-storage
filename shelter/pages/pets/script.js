...
const petImgs = document.querySelectorAll('.pet-img')
const petImg = document.querySelector('.pet-img')
const petNames = document.querySelectorAll('.pet-name')
const petName = document.querySelector('.pet-name')
const nextBtn = document.getElementById('next')
const prevBtn = document.getElementById('prev')
const nextBtnDouble = document.getElementById('double-next')
const prevBtnDouble = document.getElementById('double-prev')
...
// 1. ПОДГОТОВИТЬ ДАННЫЕ ДЛЯ СТРАНИЦ (СДЕЛАТЬ МАССИВ СОСТОЯЩИЙ ИЗ 6-ТИ МАССИВОВ, ВНУТРИ КОТОРЫХ БУДУТ ШАФЛИТЬСЯ ОБЪЕКТЫ )
// 2. НАЛАДИТЬ МЕХАНИЗМ ВЫБОРКИ ЭТИХ СТРАНИЦ (НАВЕСИТЬ СЛУШАТЕЛИ НА КНОПКИ)
// 3. ОТРИСОВАТЬ НУЖНЫЕ ЭЛЕМЕНТЫ


function shuffleArray(array) {
    let resultedArr = array;
    for (let i = array.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        console.log(j,'j');
        [resultedArr[i], resultedArr[j]] = [resultedArr[j], resultedArr[i]];
...
function makeArr () {
    let shuffledPets 
    console.log(shuffleArray(pets))
    console.log(shuffleArray(pets))
    console.log(shuffleArray(pets))
    console.log(shuffleArray(pets))
    for (let i = 0; i < 6; i++) {
        shuffledPets = shuffleArray(pets)
...
function choosePage () {
    scorer.innerHTML = `${currentPage}`
    currentPage++ 
    let i = 1;
    petCards.forEach((el) => {
        el.children[0].src = pages[i][currentValueImg].img
        currentValueImg++
     ...
     petCards.forEach((el) => {
        el.children[1].innerHTML = pages[i][currentValueText].name
        currentValueText++
...
nextBtn.addEventListener('click', choosePage)
...
//     currentPage++   
//     petName.forEach((el) => {
//         el.innerHTML = pages[arr[arr[currentValueText]]].name
//         currentValueText++
//         if (currentValueText > 7) {
//             currentValueText = 0
//         }
//     })
//     petImg.forEach((el) => {
//         el.src = pages[currentValueImg].img
//         currentValueImg++
//         if (currentValueImg > 7) {
//             currentValueImg = 0
//         }
